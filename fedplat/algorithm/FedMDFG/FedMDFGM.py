...
def get_fedmdfgm_d(
    ...
    # === 2. 随机投影降维 ===
    if use_projection:
        # 生成固定投影矩阵（需在类中保存以避免重复生成）
        print(m,original_dim,projected_dim)
        ...
        print(grads_proj.size())
    else:
        grads_proj = grads_sparse
        projected_dim = original_dim

    # === 3. 公平性修正 ===
    fair_guidance_vec = fair_guidance_vec[topk_indices]
    value_norm = value[topk_indices] / torch.norm(value[topk_indices])
    cos = torch.clamp(value_norm @ fair_guidance_vec, -1, 1)
    bias = torch.acos(cos) * 180 / np.pi
    pref_active = (bias > alpha) or force_active

    if pref_active:
        # 计算正交修正向量 h_vec
        h_vec = (fair_guidance_vec @ value_norm * value_norm - fair_guidance_vec)
        h_vec /= torch.norm(h_vec)
        fair_grad = h_vec @ grads_proj  # (1, projected_dim)
